...
namespace server.Controllers
{
    ...
    public class TrainerDetailController : ControllerBase
    {
        ...
        public ActionResult Update([FromRoute] string Fullname, [FromBody] Modules.TrDetails tr)
        {
            try
            {
                if (!string.IsNullOrEmpty(Fullname))
                {
                    _logic.UpdateTrDetails(Fullname, tr);
                    return Ok(tr);
                }
                else
                    return BadRequest($"something wrong with {tr.Fullname} input, please try again!");
            }
            catch (SqlException ex)
            {
                return BadRequest(ex.Message);
